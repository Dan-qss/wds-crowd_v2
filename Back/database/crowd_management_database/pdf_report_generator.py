...
class PDFReportGenerator:
    ...
    def generate_pdf(self, report_data: Dict) -> BytesIO:
        ...
        # Data Quality
        story.append(Spacer(1, 0.3*inch))
        story.extend(self._create_data_quality(report_data))
    ...
    def _create_title_page(self, report_data: Dict) -> List:
        ...
        date_info = [
            ...
            ['Total Zones:', str(metadata.get('total_zones', 0))],
        ]
    ...
    def _create_executive_summary(self, report_data: Dict) -> List:
        ...
        if peak_hour:
            ...
            summary_data.append(['Peak Hour Avg People:', f"{peak_hour.get('avg_people', 0):.1f}"])
    ...
    def _create_daily_statistics(self, report_data: Dict) -> List:
        ...
        for day_stat in daily_stats:
            ...
            summary_row = [
                ['Metric', 'Value'],
                ['Total Measurements', f"{summary.get('total_measurements', 0):,}"],
                ['Total People', f"{summary.get('total_people', 0):,}"],
                ['Average People', f"{summary.get('avg_people', 0):.2f}"],
            ...
            if hourly_breakdown:
                ...
                hourly_data = [['Hour', 'Total People', 'Avg People', 'Avg %', 'Max People']]
                for hour_data in hourly_breakdown[:14]:
                    hourly_data.append([
                        hour_data.get('hour_label', ''),
                        str(hour_data.get('total_people', 0)),
                        f"{hour_data.get('avg_people', 0):.2f}",
                        f"{hour_data.get('avg_percentage', 0):.2f}%",
                        str(hour_data.get('max_people', 0))
                    ])
                
                hourly_table = Table(hourly_data, colWidths=[1.2*inch, 1.2*inch, 1.2*inch, 1.2*inch, 1.2*inch])
    ...
    def _create_zone_analysis(self, report_data: Dict) -> List:
        ...
        for zone in zones:
            ...
            zone_data = [
                ...
                ['Total Measurements', f"{zone.get('total_measurements', 0):,}"],
                ['Average People', f"{zone.get('avg_people', 0):.2f}"],
            ...
            if camera_details:
                ...
                story.append(Paragraph("Camera Details", self.styles['CustomBodyText']))
                
                cam_data = [['Camera ID', 'Area', 'Capacity', 'Avg People', 'Avg %', 'Max People']]
                for cam in camera_details:
                    cam_data.append([
                        str(cam.get('camera_id', '')),
                        cam.get('area_name', ''),
                        str(cam.get('capacity', 0)),
                        f"{cam.get('avg_people', 0):.2f}",
                        f"{cam.get('avg_percentage', 0):.2f}%",
                        str(cam.get('max_people', 0))
                    ])
                
                cam_table = Table(cam_data, colWidths=[1*inch, 1.2*inch, 1*inch, 1*inch, 1*inch, 1*inch])
    ...
    def _create_camera_analysis(self, report_data: Dict) -> List:
        ...
        if cameras:
            cam_summary_data = [['Camera ID', 'Camera Name', 'Zone', 'Capacity', 'Avg People', 'Avg %', 'Max People', 'Peak Hour']]
            for cam in cameras:
            ...
            cam_summary_table = Table(cam_summary_data, colWidths=[0.8*inch, 1.5*inch, 1*inch, 0.8*inch, 0.8*inch, 0.8*inch, 0.8*inch, 0.8*inch])
            ...
            for cam in cameras[:5]:
                ...
                story.append(Paragraph(f"Camera {cam.get('camera_id', '')}: {cam_name}", self.styles['SubsectionHeader']))
                ...
                cam_stats = [
                    ...
                    ['Total Measurements', f"{cam.get('total_measurements', 0):,}"],
                    ['Average People', f"{cam.get('avg_people', 0):.2f}"],
