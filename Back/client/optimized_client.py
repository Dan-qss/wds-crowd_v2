...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/camera_client.log'),
...
class OptimizedCameraClient:
    def __init__(self, camera_ids=["1", "2", "3", "4", "5"]):
        ...
        logger.info("Loading Model1...")
        self.model = Model1(self.db)  # Pass database connection to Model1
        logger.info("Model1 loaded successfully")
    ...
    def start_camera_stream(self, camera_id):
        """Handle streaming for a single camera"""
    ...
    def process_frames(self):
        """Process frames from all cameras using Model1"""
            ...
            while self.running:
                for camera_id in self.camera_ids:
                    try:
                        ...
                        # Get additional camera info from database
                        camera_info = next(
                        ...
                        self.processing_times[camera_id].append(process_time)
                        
                        if len(self.processing_times[camera_id]) >= 100:
                            avg_time = sum(self.processing_times[camera_id]) / 100
